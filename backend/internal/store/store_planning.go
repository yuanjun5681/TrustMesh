...
// GetTaskPMNode returns the PM agent node ID for a planning task. Used by handler to send task.message.
func (s *Store) GetTaskPMNode(userID, taskID string) (string, *transport.AppError) {
	s.mu.RLock()
	defer s.mu.RUnlock()
