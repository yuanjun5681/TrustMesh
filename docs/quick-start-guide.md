...
如果你还没有现成的 Agent，可以先通过「岗位市场」选择合适的角色包，再安装到本地的 OpenClaw 节点中。

**第一步：进入岗位市场**

1. 登录后，点击左侧导航栏的「**岗位市场**」
2. 页面左侧可按部门筛选角色，右上角可按关键词搜索角色名称或描述
3. 点击任意角色卡片进入详情页

**第二步：查看角色详情**

角色详情页包含三部分内容，方便你判断这个角色是否适合加入团队：

| 标签 | 内容 |
|------|------|
| **简介** | 角色定位、擅长领域、适用场景 |
| **人格** | 角色的思考方式、价值观和工作风格 |
| **工作规范** | 角色的执行流程、输出要求和约束条件 |

你可以优先选择以下几类角色：

- 1 个适合作为项目经理的角色，用于负责需求澄清和任务拆解
- 1 个或多个执行型角色，用于处理开发、设计、测试、运营等具体工作

**第三步：安装角色包**

1. 在角色详情页右侧「安装说明」区域点击「**复制提示词**」
2. 打开对应机器上的 OpenClaw 对话窗口，粘贴并发送这段提示词
3. OpenClaw 会按提示词中的角色包地址自动完成下载和安装

如果你希望手动处理文件，也可以再使用右上角的「**下载角色包**」按钮，将 zip 文件下载到本地后交给 OpenClaw 安装，但这不是必需步骤。

> **提示**：岗位市场里的角色包是 OpenClaw Agent 格式。默认推荐直接复制提示词安装。建议 PM Agent 和执行 Agent 分别选择不同角色，避免团队职责重叠。
...
> **建议流程**：先从「岗位市场」下载并安装角色包，再到「招聘智能体」页面发起入职和审批，这样 Agent 的角色定位会更清晰。
