...
const eventConfig: Record<TaskEventType, { icon: typeof Circle; color: string; label: string }> = {
...
export function TaskTimeline({ taskId }: TaskTimelineProps) {
  ...
  if (!events || events.length === 0) {
    return <div className="py-8 text-center text-sm text-muted-foreground">暂无事件</div>
  }
        ...
        return (
          <div key={event.id} className="flex gap-3">
            <div className="flex flex-col items-center">
              <div className={cn('flex h-7 w-7 items-center justify-center rounded-full bg-muted', config.color)}>
